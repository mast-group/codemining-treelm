...
import codemining.lm.tsg.samplers.AbstractCollapsedGibbsSampler;
...
public class CollapsedGibbsSamplerTest {
	...
	public void getPosteriorForTree() {
		...
		assertEquals(
				sampler.getSampleGrammar().computeRulePosteriorLog2Probability(
	...
	public void testGetPosteriorProbabilityForTree() {
		...
		assertEquals(
				sampler.getPosteriorComputer()
						.getPrior()
						.getLog2ProbForCFG(
								new AbstractContextFreeGrammar.CFGRule(1, nc)),
				0, 0);
		assertEquals(
				sampler.getPosteriorComputer()
						.getPrior()
						.getLog2ProbForCFG(
								new AbstractContextFreeGrammar.CFGRule(2, nc2)),
				-1, 0);
	...
	public void testSampler(final TreeNode<TSGNode> root,
		...
		for (int i = 0; i < 10000; i++) {
			...
			if (toBeSampled.getData().isRoot) {
				countRoot++;
				assertEquals(
						sampler.getSampleGrammar()
								.computeRulePosteriorLog2Probability(
